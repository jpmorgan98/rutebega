...
axs[1].text(.15, 0.4, r"$\rho$>1.0", rotation='vertical', color='black', fontsize=14)
...
# ---- plotting helper ----
def plot_pair(title, err_phi, err_psi, mess):
    ...
    fig, axs = plt.subplots(1, 2, figsize=(13, 6), constrained_layout=False)
    ...
    if mess:
        axs[0].text(.15, 0.4, r"$\rho$>1.0", rotation='vertical', color='white', fontsize=14)
        axs[1].text(.15, 0.4, r"$\rho$>1.0" , rotation='vertical', color='white' , fontsize=14)
...
# ---- figure 1: rescale ----
plot_pair("Rescale vs transport", err_phi_rescale, err_psi_rescale, mess=False)
plt.savefig(f"err_rescale_{metric}.pdf", dpi=200)

# ---- figure 2: yavuz ----
plot_pair("Yavuz vs transport", err_phi_yavuz, err_psi_yavuz, mess=True)
plt.savefig(f"err_yavuz_{metric}.pdf", dpi=200)
